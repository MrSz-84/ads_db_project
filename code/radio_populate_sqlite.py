...
print('---Radio populate sqlite part---')

file_path = cf.SOURCE_PATH


def get_index_val(table_name: str, cur: sqlite3.Cursor) -> int:
    """
    Function gets max index value from the selected table and returns it as an integer increased
    by one. When the table is empty, returns 1
    ...
    locked
    :return: number representing max index value of selected table increased by 1
    :rtype: int
    """
...
def add_1_field(data: list, table_name: str, field_name: str,
    ...
    :param field_name: String representing the name of field / column in destination table
    :param con: Is a connection object, pointing to a DB
    :param cur: Is a cursor object created for con object
    :raise sqlite3.ProgrammingError: If there are any error raised by the DB-API
    :raise sqlite3.OperationalError: If any exceptions on the DB side are raised, i.g. DB being
...
def check_for_data_1_field(data_: list, table_name: str, field_name: str,
    ...
    if len(in_db) == 0:
        ...
        if table_name == 'kody_rek':
            ...
            new_data.loc[:, field_name] = (
                new_data.loc[:, ['a', 'b']].apply(lambda x: f'{x["a"]}@|@{x["b"]}', axis=1))
...
def add_3_fields(data_set: dict[str, pd.DataFrame | list[str] | str], con: sqlite3.Connection,
                 cur: sqlite3.Cursor) -> None:
    """
    Function adding data into submedia table, which consists of 3 columns.
...
def get_id_for_submediums(fields_: list[str], table_: str,
    ...
    Returns a bool for logic purposes and data to be added into submedium.
...
def check_for_data_3_fields(fields: list[str], table_name: str, submedia: pd.DataFrame,
    ...
    If DB is empty returns original DF. During data update process returns the data not present
    in the DB or indicates there is nothing to be added.
...
def add_14_fields(data_set: dict[str, pd.DataFrame | list[str] | str],
                  con: sqlite3.Connection, cur: sqlite3.Cursor) -> None:
    """
    Function adding data spoty table, which consists of 14 columns.
...
def get_id_for_ads_desc(fields: list[str], table_: str,
                        cur: sqlite3.Cursor) -> tuple[bool, pd.DataFrame]:
    """
    Gets IDs from reference tables to spoty table.
    ...
    Returns a bool for logic purposes and data to be added into spoty.
    ...
    spoty = df[['Data', 'GG', 'MM', 'SS', 'Koszt [zł]', 'dł./mod.', 'Kod Reklamy', 'Daypart',
                'dł. Ujednolicona',
                'Godzina bloku reklamowego', 'Brand', 'Submedium', 'Produkt(4)', 'L.emisji',
                'Typ reklamy', 'Zlepek']]
    ...
    spoty.loc[:, 'Godzina bloku reklamowego'] = spoty['Godzina bloku reklamowego'].map(
        blok_rek_id).astype('object')
...
df['Data_str'] = df['Data'].dt.strftime('%Y-%m-%d')
df['Zlepek'] = df.apply(
    lambda x: f"""{str(x['Data_str']) + str(x['GG']) + str(x['MM']) + str(x['SS'])
                   + str(x['dł./mod.']) + str(x['Kod Reklamy']) + x['Brand'] + x['Submedium']}""",
    axis=1)
df.drop(labels='Data_str', axis=1, inplace=True)
df_end = time.time()
...
ad_codes = df.loc[:, ['Kod Reklamy', 'Opis Reklamy']].drop_duplicates(subset=['Kod Reklamy'],
                                                                      keep='first',
                                                                      ignore_index=True)
...
Threes processing time   : {three_diff:.2f}
Fourteens processing time: {fourteen_diff:.2f}
""")
