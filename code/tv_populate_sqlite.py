...
print('---TV populate sqlite part---')

file_path = find.find(path=cf.FILE_PATH)
...
def add_16_fields(data_set: dict[str, pd.DataFrame | list[str] | str],
                  con: sqlite3.Connection, cur: sqlite3.Cursor) -> None:
    """
    Function adding data into spoty table, which consists of 14 columns.
...
def get_id_for_spoty(fields: list[str], table_: str,
    ...
    spoty.loc[:, 'Prog Before'] = spoty['Prog Before'].map(program_przed_id).astype('str')
    spoty.loc[:, 'Prog After'] = spoty['Prog After'].map(program_po_id).astype('str')
...
dfs = pd.read_excel(file_path, sheet_name='Spotlista', thousands=' ', decimal=',', dtype={
                    'Month': 'category', 'Week': 'category', 'Weekday': 'category',
                    'Time': 'string', 'Dayparts': 'object', 'Channel Groups': 'object', 
                    'Channel': 'object', 'PIB pos': 'int8', 'PIB (real) rel': 'object',
                    'PIB count': 'int16', 'Dur rounded,sp': 'int8', 'Spot Class': 'object',
                    'Block Code': 'object', 'Syndicate': 'object', 'Producer': 'object',
                    'Brand': 'object', 'Film Code': 'object', 'Prog Campaign': 'object',
                    'Prog Before': 'object', 'Prog After': 'object', 
                    'Film Code/2': 'object', 'TRP All 40+ cities 200- (i wsie) Nat[TSV2 , IN+OOH]': 'float64'},
                    parse_dates=['Date']
                    )
# df_tv = pd.concat(dfs, ignore_index=True)
df_tv = dfs
# del dfs
df_tv['Date'] = pd.to_datetime(df_tv['Date'], dayfirst=True, format='%Y-%m-%d')
df_tv['Time'] = df_tv['Time'].str[-8:]
df_tv.rename(columns={'TRP All 40+ cities 200- (i wsie) Nat[TSV2 , IN+OOH]': 'GRP'}, inplace=True)
df_tv['Brand'] = df_tv['Brand'].str.strip()
df_tv['Brand'] = df_tv['Brand'].str.upper()
df_tv['Producer'] = df_tv['Producer'].str.upper()
df_tv['Syndicate'] = df_tv['Syndicate'].str.upper()
...
df_tv['Kanal Grupa'] = df_tv['Channel'] + '@|@' + df_tv['Channel Groups']
df_tv['Brand Prod Synd'] = df_tv['Brand'] + '@|@' + df_tv['Producer'] + '#|#' + df_tv['Syndicate']
df_tv['Zlepek'] = df_tv['Date'].dt.strftime('%Y-%m-%d').astype('str') + df_tv['Time'].astype('str') + df_tv['Brand'] \
                  + df_tv['Channel'] + df_tv['Film Code/2'].astype('str')
# df_tv['DateTime'] = pd.to_datetime(df_tv['DateTime'], format='ISO8601')
df_tv[['Dayparts', 'Prog Before', 'Prog After']] = df_tv[['Dayparts', 'Prog Before', 'Prog After']].fillna('brak danych', axis=1)
df_tv.sort_values(by='Date', inplace=True, axis=0)
df_tv.reset_index(inplace=True)
...
Sixteens processing time: {fourteen_diff:.2f}
""")
